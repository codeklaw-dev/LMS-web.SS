 ...
 * Quotes are marked `placeholder: true` until real references are signed
 * off; the flag renders visibly so nothing fictional ships by accident.
 */
export default function Testimonials({
  dict,
  locale,
  items = dict.testimonials,
  caseStudy = dict.featuredCaseStudy,
}: {
  dict: Dictionary
  locale: Locale
  items?: Dictionary['testimonials']
  caseStudy?: Dictionary['featuredCaseStudy'] | null
}) {
  return (
    <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
      {items.map((t, i) => (
        <Reveal key={t.school} delay={i * 0.06}>
          <Card panel locale={locale} style={{ height: '100%' }}>
            {t.placeholder && (
              <Badge
                tone="outline"
            ...
            <div className="attrib">
              <span className="attrib__avatar" aria-hidden="true">
                {initials(t.name)}
              </span>
              <div>
                <p className="attrib__name">{t.name}</p>
                <p className="attrib__meta">
                  {t.role} · {t.school} · {t.country}
                </p>
              </div>
            </div>
          </Card>
      ...
      {caseStudy && (
        <Reveal delay={0.12}>
          <Card
            to={caseStudy.href}
            locale={locale}
            style={{ height: '100%', background: 'var(--bg-inset)' }}
          >
            ...
            <dl className="grid grid--3" style={{ gap: 'var(--space-4)', marginTop: 'var(--space-6)' }}>
              {caseStudy.stats.map((s) => (
                <div className="stat" key={s.label}>
                  <dt className="visually-hidden">{s.label}</dt>
                  <dd className="stat-value" style={{ margin: 0, fontSize: 'var(--step-2)' }}>
                    {s.value}
                  </dd>
